...
export function renderHookFlash(slots: Record<string, any>, width: number, height: number): string {
  const eyebrow = slots.eyebrow ? `<div class="gsap-eyebrow" style="font-size: ${aisTokens.typography.label.fontSize}; color: ${aisTokens.colors.primary}; text-transform: uppercase; letter-spacing: 0.2em; margin-bottom: ${aisTokens.spacing.sm}; text-shadow: ${aisTokens.effects.glowMild};">${escapeHtml(slots.eyebrow)}</div>` : "";
  const title = slots.title || "TITLE";
